...
# Откуда пришла попытка отправки. «auto» — автоматический путь: режим обязателен.
# «manual» — человек сам заказал отправку одобренного черновика (PLAN 16.11): режим
# здесь не проверяется, потому что предохранители другие — право DRAFT_SEND (владелец
# и заказчик, не разборщик), явное подтверждение в интерфейсе и аудит каждой попытки.
# Остальные гардрейлы (потолок, пауза, тихие часы, «уже писали») действуют в обоих
# режимах — ручная отправка не отменяет правил приличия, только режим.
ORIGINS = ("auto", "manual")
...
def check_all(
    *,
    ...
    `origin="manual"` не вызывает `not_in_dry_run` — ручная отправка идёт мимо
    проверки режима (решение владельца, PLAN 16.11): вместо неё у попытки другие
    предохранители, и перечислены они в комментарии к `ORIGINS` выше. Значение
    `mode` при этом всё равно приходит вызывавшим — гейт читает его одним и тем же
