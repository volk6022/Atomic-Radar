...
# Маскировка переехала в `services/engage.py`: та же подпись нужна очереди
# черновиков, а две разные означали бы закрытый номер на одном экране и открытый
# на соседнем. Имя здесь сохранено — на него ссылаются тесты флота.
_mask_phone = engage.mask_phone
