...
class JoinDiscussionsRequest(BaseModel):
    ...
    channel_ids: list[int] = Field(default_factory=list)
    account_ids: list[int] = Field(default_factory=list)
    # Верхняя граница запроса: сколько групп максимум планируется на аккаунт
    # в этом прогоне. Настоящий потолок — остаток Engage (`GET /v1/limits`), его
    # прогон спрашивает сам при планировании. `None` — «сколько позволит Engage».
    per_account: int | None = Field(None, ge=1)
...
async def join_discussions(body: JoinDiscussionsRequest, request: Request, db: GetDB,
    ...
    * **потолок пачки — остаток Engage, не константа.** Сверх лимита Engage задачу
      не отвергает, а откладывает на час и переносит, пока не сменятся сутки;
      заказать сорок вступлений разом — значит получить сорок задач, сутками
      стучащихся в планировщик. Поэтому перед планированием прогон один раз
      спрашивает `GET /v1/limits` и режет пачку по остатку на аккаунт и по
      агрегату api-группы. `per_account` в теле — только верхняя граница одного
      запроса; `None` значит «сколько позволит Engage».
