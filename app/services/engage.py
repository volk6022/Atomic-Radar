...
def webhook_url(**params) -> str:
    ...
    if not s.INGEST_TOKEN:
        # Без токена адрес собирается как `/api/v1/ingest/?kind=…`, приёмник отвечает
        # 404, а Engage считает это временной неудачей и ретраит пять раз. То есть
        # процесс без токена не отказывает, а тихо плодит недоставленные вебхуки —
        # ровно так 04.09 воркер прогонов заказывал вступления без `RADAR_INGEST_TOKEN`
        # в окружении. Падать здесь честнее: адрес возврата обязателен по контракту
        # Engage даже там, где результат забирается опросом.
        raise RuntimeError(
            "RADAR_INGEST_TOKEN не задан, а без него адрес возврата вебхука "
            "невозможен: Engage получит путь без секрета и будет ретраить 404")
