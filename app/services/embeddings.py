...
# Сколько символов текста уходит в эмбеддер. Ограничение не наше, а сервера: текст
# длиннее физической пачки llama.cpp не берёт вовсе — отвечает 500 на весь запрос.
# 28.08 на этом оборвалась переклассификация всего прода: одно сообщение на 535
# токенов уронило прогон по 16 тысячам.
#
# С 08.09 пачка поднята до 2048 токенов (`-ub 2048` в конфигах обеих карт), и предел
# вырос вместе с ней. Это не украшение: замер 07.09 показал, что настоящие запросы
# длинные — медиана сообщения, дошедшего до L3, 328 символов против 55 у короткого
# шума, а близость к эталону у коротких текстов завышена и неустойчива. Резать
# ровно там, где начинается содержательная часть, значило бы кормить L2 заголовками.
#
# Две тысячи символов русского — это 550–600 токенов, то есть втрое ниже пачки.
# Запас взят намеренно: 8000 символов (2196 токенов) сервер уже отвергает, и
# граница должна лежать далеко от обрыва, а не рядом с ним. Поднимать этот предел
# можно только вместе с `-ub`; опускать `-ub`, не опустив предел, — вернуть тот
# самый отказ 500 (договор описан в `ecosystem.embed.config.js` и `_shared.js`).
