...
ASKS_FOR_CONTRACTOR = "ищет, через кого платить"
...
PUBLIC_TEMPLATES: Mapping[str, tuple[str, ...]] = MappingProxyType({
    "не может оплатить за рубеж": (
        "Тут первым делом стоит развести две разные задачи: разовый платёж и "
        "регулярные. Под разовый обычно не нужен ни валютный счёт, ни контракт на "
        "учёте — а под регулярные без них дороже выйдет.",
        "Смотря что оплачиваете: товар, услугу или лицензию. От этого зависит и "
        "пакет документов, и то, попадёт ли платёж под требование о постановке "
        "контракта на учёт.",
        "Если счёт уже выставлен, стоит посмотреть, кто в нём получатель и в какой "
        "стране банк. Часть направлений закрыта не нашими правилами, а санкционными, "
        "и это меняет весь дальнейший путь.",
    ),
    "банк не пропускает платёж": (
        "Стоит уточнить, на чём именно остановились: банк запросил документы или "
        "отказал по комплаенсу. Это две разные истории — первая решается бумагами, "
        "вторая сменой маршрута.",
        "Если сумма до порога, контракт на учёт ставить не требуется, и банк иногда "
        "просит его по инерции. Полезно спросить письменно, на каком основании.",
        "Возврат платежа обычно приходит с кодом причины. По нему видно, чей это был "
        "отказ — вашего банка, корреспондента или банка получателя, — и дальше "
        "действия совсем разные.",
    ),
    "выплаты людям за границей": (
        "Ключевой вопрос — статус человека: нерезидент, самозанятый или ИП в другой "
        "стране. От него зависит и налоговая часть, и то, можно ли платить с "
        "расчётного счёта вообще.",
        "Если человек работает больше полугода подряд, риск переквалификации в "
        "трудовые отношения растёт независимо от способа выплаты. Это стоит держать "
        "в голове до выбора маршрута, а не после.",
        "Стоит разделить: разовая выплата подрядчику и ежемесячная зарплата "
        "релоканту решаются по-разному, хотя выглядят одинаково.",
    ),
    "оплата рекламы и западных сервисов": (
        "Смотря чей кабинет: собственный аккаунт компании или агентский. Во втором "
        "случае пополнение обычно решается на стороне агентства, и отдельный "
        "платёжный маршрут не нужен.",
        "Если карта не проходит именно на этапе привязки, дело чаще в стране "
        "выпуска, а не в самой компании. Проверяется быстро — другой картой.",
        "Стоит заранее решить, нужны ли закрывающие документы под эти расходы. "
        "Если да, вариантов меньше, и выбирать лучше сразу из них.",
    ),
    # Единственная боль, где контакт уместен публично: его и спросили.
    ASKS_FOR_CONTRACTOR: (
        "Смотря что нужно: разовый платёж или постоянный поток. Под разовый обычно "
        "проще найти того, кто закроет конкретный счёт, под поток — работать по "
        "договору с фиксированной комиссией.",
        "Стоит сразу спросить про закрывающие документы и про то, в какие страны "
        "проходит: у всех разный список, и это главная развилка.",
        f"Могу порекомендовать: Андрей ({CONTACT}), проводил нам платежи за рубеж — "
        "и провёл, и объяснил, как это ложится в учёт.",
    ),
    "нет валютного счёта или контракта": (
        "Открывать валютный счёт ради одного платежа обычно невыгодно: обслуживание "
        "и время на комплаенс съедают больше, чем сам платёж. Под разовые задачи есть "
        "маршруты без него.",
        "Если контракта с иностранцем нет вовсе, то и ставить на учёт нечего — "
        "требование банка в этом случае стоит уточнить письменно.",
        "SWIFT и SEPA — это про способ доставки, а не про допустимость платежа. "
        "Сначала имеет смысл понять, проходит ли направление в принципе.",
    ),
})

PUBLIC_FALLBACK = (
    "Опишите чуть подробнее, что именно оплачиваете и на чём остановилось — по такой "
    "вводной обычно видно, какой маршрут вообще подходит.",
)
