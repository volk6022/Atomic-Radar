...
def test_reason_lookup_is_closed():
    ...
    Справочник стал общим на оба контура (`wf_queues` берёт его отсюда же), поэтому
    имя публичное. Проверка от этого не изменилась: закрытым список обязан остаться
    именно потому, что им теперь пользуются двое.
    """
    assert drafts.REASON_BY_N.keys() == {1, 2, 3, 4, 5, 6, 7, 8, 9}
...
def test_spam_score_reacts_to_advertising_markers():
    clean = drafting.spam_score(
        "Видел твоё сообщение про оплату инвойса — помогло своё решение")
