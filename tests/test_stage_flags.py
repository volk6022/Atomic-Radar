...
перевода одно на всё приложение и живёт в `cascade.stage_flags`; здесь оно закрыто
...
from app.core.cascade import stage_flags


def test_dropped_at_l0():
    assert stage_flags(0, False) == {"l0": False, "l1": None, "l2": None, "l3": None}


def test_dropped_at_l1_keeps_l0_passed():
    assert stage_flags(1, False) == {"l0": True, "l1": False, "l2": None, "l3": None}
...
def test_dropped_at_l2_keeps_l0_and_l1_passed():
    ...
    assert stage_flags(2, False) == {"l0": True, "l1": True, "l2": False, "l3": None}


def test_passed_everything():
    assert stage_flags(3, True) == {"l0": True, "l1": True, "l2": True, "l3": True}
...
def test_stopped_because_next_stage_is_off():
    ...
    assert stage_flags(1, True) == {"l0": True, "l1": True, "l2": None, "l3": None}
...
def test_in_flight_looks_the_same_as_stopped():
    ...
    assert stage_flags(1, None) == {"l0": True, "l1": True, "l2": None, "l3": None}
    assert stage_flags(2, None) == {"l0": True, "l1": True, "l2": True, "l3": None}


def test_never_processed():
    assert stage_flags(None, None) == {"l0": None, "l1": None, "l2": None, "l3": None}
