...
def test_unbuilt_stages_are_not_silently_passed():
    """L2/L3 ещё нет. Их отсутствие должно быть написано словами, иначе пустая
    строка на экране прочитается как «ступень пройдена»."""
    v = classify("не могу настроить 3x-ui на дебиане, помогите разобраться")
    assert v["passed"] is True
    assert "не подключены" in v["detail"]["l2"]
    assert "не подключена" in v["detail"]["l3"]
