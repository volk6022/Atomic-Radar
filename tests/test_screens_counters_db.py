"""Счётчик «Черновиков в очереди» видит только сценарную очередь — на настоящем Postgres.

Плитка дашборда, элемент очередей и бейдж меню читают одно и то же число из
`_counts`. Раньше к черновикам сценариев (`wf_drafts`) прибавляли старый
контур (`drafts`), но с 13.09 тот спрятан из панели и остался архивом по
прямой ссылке, а его pending — дубли тех же лидов: воркер старого контура
заводит их лениво в сценарную очередь (стенд 14.09: 1222 в двойном счётчике
против 1034 в сценариях). Считать старый контур — значит показывать очередь,
которой нет ни в одном списке. Проверять это можно только целиком: счётчик,
разбивка по сценариям и плитка — разные места одного ответа, и разойтись они
могут независимо.
...
а не только «ждёт ревью», на таких данных уезжает в 3 вместо 2.
...
async def _seed() -> dict:
    ...
    оператора. Старый контур: ровно один pending — теперь как то, что счётчик
    НЕ должен считать: контур с 13.09 архивен, а его pending дублируют те же
    лиды в сценарных очередях.
    """
...
def test_counters_ignore_the_legacy_contour(authed):
    """Бейдж меню: 1 cold_dm + 1 public_reply, старый pending не в счёт.

    Старый контур с 13.09 спрятан и архивен, а его pending — дубли тех же
    лидов в сценарных очередях: прибавить его — показать 3 там, где ждут
    ровно 2.
    """
    assert authed.get("/api/v1/counters").json()["drafts"] == 2
...
def test_dashboard_breaks_the_queue_down_by_workflow(authed):
    ...
    # Старого слагаемого больше нет, поэтому разбивка покрывает счётчик целиком:
    # сумма разбивки и бейдж меню — одно и то же число.
    assert authed.get("/api/v1/counters").json()["drafts"] == 2
