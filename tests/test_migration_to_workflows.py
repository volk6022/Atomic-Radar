...
async def test_lead_with_a_draft_cannot_be_deleted(db):
    ...
    Та же особенность однажды роняла переклассификацию: `_reconcile_leads` удалял лид
    в статусе `new`, переставший проходить каскад, а черновик у нового лида — норма
    (`ensure_draft` заводит его как раз новым). Прогон падал на первом же таком лиде и
    откатывался целиком. Починено: неразобранный черновик удаляется вместе с лидом,
    разобранный оставляет лид на месте — см. `tests/test_reclassify_leads_db.py`.

    Здесь проверяется само ограничение базы, а не поведение переклассификации: если
    `ondelete` однажды появится, об этом надо узнать отсюда.
    """
