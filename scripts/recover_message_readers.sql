...
--
-- ⚠️ ЧЕГО ЭТОТ СПОСОБ НЕ ВОССТАНОВИТ. Живой поток вотчера: апдейты приходят
-- пушем, задачи под них не заводится, и в журнале Engage их нет. Это ~1700
-- сообщений россыпью с 28.08 по 01.09. Ни одного черновика среди них не
-- оказалось; если появятся — восстанавливать будет нечем.
--
...
-- пометки: у восстановленных `first_seen_at` — момент чтения по журналу Engage
-- (не позже 2026-09-01 18:36:54Z), у живых — не раньше 2026-09-02 15:18Z.
--     delete from message_readers where first_seen_at < '2026-09-02';
--
